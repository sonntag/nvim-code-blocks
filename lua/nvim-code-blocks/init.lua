...
function M.get_containing_block()
    ...
    -- When cursor is in virtual space (past end of line), clamp it to the last character
    -- This ensures we find the block that actually ends on this line, not a parent
    local current_line = vim.api.nvim_buf_get_lines(bufnr, row, row + 1, false)[1] or ""
    local search_col = col
    if col >= #current_line and #current_line > 0 then
        search_col = #current_line - 1
    end

    local node = root:named_descendant_for_range(row, search_col, row, search_col)
