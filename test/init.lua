-- Test configuration for nvim-code-blocks
-- This file is sourced AFTER your main neovim config
...
vim.keymap.set("n", "<leader>ct", function()
	...
	if block then
		print(string.format("Found block: %s (lines %d-%d)", block.node:type(), block.start_row + 1, block.end_row + 1))
	else
		...
		if trees and #trees > 0 then
			...
			while temp_node do
				...
				if sr == row then
					...
					print(
						string.format(
							"  type=%s, start_byte_col=%d, start_display_col=%d",
							node_type,
							sc,
							start_display_col
						)
